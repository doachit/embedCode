/*
	* @Author       : LaiGuo_Copy_By_Net
	* @Date         : 2024-10-13 13:40:34
	* @LastEditTime : 2024-10-13 15:36:57
	* @CSDN  				: https://blog.csdn.net/m0_58326153/
	* @Bili  				: https://b23.tv/llaF6l2
*/
#include "iot_usart2.h"     //包含需要的头文件
...
{
    if((USART_GetITStatus(USART2, USART_IT_RXNE) != RESET) && (ConnectALi_flag == 0))        //如果USART_IT_RXNE标志置位，表示有数据到了，且当前已没有经连接上服务器 进入if分支
    {
    ...
    if((USART_GetITStatus(USART2, USART_IT_IDLE) != RESET) && (ConnectALi_flag == 1))        //如USART_IT_IDLE标志置位，表示空闲中断	且当前已经连接上服务器 进入if分支
    {
